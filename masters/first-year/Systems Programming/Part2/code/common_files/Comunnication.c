...
void *create_server_channel(void *ctx, const char *port)
{
    ...
    char endpoint[64];
    snprintf(endpoint, sizeof(endpoint), "tcp://*:%s", port);

    int response = zmq_bind(responder, endpoint);
    if (response != 0) {
        printf("Error binding socket (%s): %s\n",
                  ...
                  direction_t *d, int *pass_out)
{
    ...
    *c = '\0';
    ...
    // Try to decode as client_exit_req
    ClientExitReq *exit_req = 
        client_exit_req__unpack(NULL, n, buffer);
    
    if (exit_req != NULL){
        if (exit_req->type != NULL)
            strcpy(message_type, exit_req->type);
        else
            strcpy(message_type, "EXIT");

        // 2. ch (bytes → your single-character name)
        if (exit_req->ch.len > 0)
            *c = exit_req->ch.data[0];
        else
            *c = '\0';

        // 3. pass field
        if (pass_out)
            *pass_out = exit_req->pass;

        client_exit_req__free_unpacked(exit_req, NULL);
        return;
    }
...
void * create_client_channel(const char * server_ip_addr, void *ctx, const char *port){
    ...
    sprintf(server_zmq_addr, "tcp://%s:%s", server_ip_addr, port);
...
//////////////////////////////////
///Sends EXIT connection request protobuf to server
/////////////////////////////////
void send_exit_message(void *fd, ship *player) {
    ClientExitReq req = CLIENT_EXIT_REQ__INIT;
    ...
    size_t size = client_exit_req__get_packed_size(&req);
    if (size == 0) {
        fprintf(stderr, "send_connection_message: packed size is 0 — nothing to send\n");
        return;
    }
    ...
    uint8_t *buffer = malloc(size);
    if (!buffer) { perror("malloc"); return; }
    client_exit_req__pack(&req, buffer);
...
//////////////////////////////////
///Sends ACTIVE connection request protobuf to server
/////////////////////////////////
