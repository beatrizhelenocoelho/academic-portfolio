classdef HelperLaneDetectorWrapper < matlab.System
    ...
    %     Camera = struct('ImageSize',[768 1024], ...
    ...
    properties
       % Camera sensor parameters
        Camera = struct('ImageSize',[480 640], ... % same as camera block
            'PrincipalPoint',[320 240],...         % same as camera block, assuming it`s the optical center
            'FocalLength',[554 554], ...           % same as camera block
            'Position',[0.57 0 1.45],...           % before: [1.8750 0 1.2000]
            'PositionSim3d',[0.57 0 1.45], ...     % before: [0.5700 0 1.2000]
            'Rotation',[0 0 0],...                 % assuming none, the camera just points foward with no tilt
            'LaneDetectionRanges',[6 30], ...      % variable we can tune
            'DetectionRanges',[6 50],...           % variable we can tune
            'MeasurementNoise',diag([6,1,1]));     % can try to play with this to see if has any effect
    end
    ...
    methods(Access = protected)
        ...
        function detections = packLaneBoundaryDetections(obj,leftEgoBoundary,rightEgoBoundary)
            ...
            % DefaultLanesLeft = struct('Curvature',{double(obj.LaneMarkerDetector.DefaultLeftLaneParams(1))},...
            %     'CurvatureDerivative',{double(0)},...
            %     'HeadingAngle',{double(obj.LaneMarkerDetector.DefaultLeftLaneParams(2))},...
            %     'LateralOffset',{double(obj.LaneMarkerDetector.DefaultLeftLaneParams(3))},...
            %     'Strength',{double(0)});
            % 
            % DefaultLanesRight = struct('Curvature',{double(obj.LaneMarkerDetector.DefaultRightLaneParams(1))},...
            %     'CurvatureDerivative',{double(0)},...
            %     'HeadingAngle',{double(obj.LaneMarkerDetector.DefaultRightLaneParams(2))},...
            %     'LateralOffset',{double(obj.LaneMarkerDetector.DefaultRightLaneParams(3))},...
            %     'Strength',{double(0)});
        ...
        % function detections = packLaneBoundaryDetections(obj,leftEgoBoundary,rightEgoBoundary)
        %     % 1. Initialize as SINGLE (You already did this part)
        %     DefaultLanesLeft = struct(...
        %         'Curvature',{single(obj.LaneMarkerDetector.DefaultLeftLaneParams(1))},...
        %         'CurvatureDerivative',{single(0)},...
        %         'HeadingAngle',{single(obj.LaneMarkerDetector.DefaultLeftLaneParams(2))},...
        %         'LateralOffset',{single(obj.LaneMarkerDetector.DefaultLeftLaneParams(3))},...
        %         'Strength',{single(0)});
        ...
        %         'Curvature',{single(obj.LaneMarkerDetector.DefaultRightLaneParams(1))},...
        %         'CurvatureDerivative',{single(0)},...
        %         'HeadingAngle',{single(obj.LaneMarkerDetector.DefaultRightLaneParams(2))},...
        %         'LateralOffset',{single(obj.LaneMarkerDetector.DefaultRightLaneParams(3))},...
        %         'Strength',{single(0)});
        ...
        %     detections = struct(field1,DefaultLanesLeft,field2,DefaultLanesRight);                     
        % 
        %     % 2. Pack Left Lane - FORCE SINGLE ON ASSIGNMENT
        %     detections.Left.Curvature(:)     = single(2 * leftEgoBoundary.Parameters(1));
        %     detections.Left.HeadingAngle(:)  = single(leftEgoBoundary.Parameters(2));
        %     detections.Left.LateralOffset(:) = single(leftEgoBoundary.Parameters(3));
        %     detections.Left.Strength(:)      = single(leftEgoBoundary.Strength);
        % 
        %     % 3. Pack Right Lane - FORCE SINGLE ON ASSIGNMENT
        %     detections.Right.Curvature(:)     = single(2 * rightEgoBoundary.Parameters(1));
        %     detections.Right.HeadingAngle(:)  = single(rightEgoBoundary.Parameters(2));
        %     detections.Right.LateralOffset(:) = single(rightEgoBoundary.Parameters(3));
        %     detections.Right.Strength(:)      = single(rightEgoBoundary.Strength);
        % 
        %     % 4. Coordinate Transform - THIS IS THE CRITICAL FIX
        %     % polyval returns DOUBLE. We must cast it to SINGLE immediately.
        %     if nnz(leftEgoBoundary.Parameters)
        %         val = polyval(leftEgoBoundary.Parameters, -obj.Camera.PositionSim3d(1));
        %         detections.Left.LateralOffset(:) = single(val); % <--- The Fix
        % 
        %         if detections.Left.LateralOffset < 0
        %             detections.Left = DefaultLanesLeft;
        %         end
        ...
        % 
        %     if nnz(rightEgoBoundary.Parameters)
        %         val = polyval(rightEgoBoundary.Parameters, -obj.Camera.PositionSim3d(1));
        %         detections.Right.LateralOffset(:) = single(val); % <--- The Fix
        % 
        %         if detections.Right.LateralOffset > 0
        %             detections.Right = DefaultLanesRight;
        %         end
