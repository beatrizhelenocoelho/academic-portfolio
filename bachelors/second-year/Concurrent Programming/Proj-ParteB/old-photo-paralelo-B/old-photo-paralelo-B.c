...
char file_ver[100];
int count_thread; 
char ** files; //nome das imagens
int *vector; // vector que guarda os tempos das threads
int copy_thread;

void*thread_function(void*arg){

	struct timespec start_time_thread, end_time_thread, start_time_img;
	struct timespec *thread_time = (struct timespec*)malloc(sizeof(struct timespec)); // estrutura para guardar o tempo das threads
	...
	int i=0;
	copy_thread--;
	int copy_t = copy_thread;
	
	while(1){ // até dar break continua 
		if (read(pipe_fd[0], &i, 4)==0) // vamos ler da pipe o indice da imagem a tratar 
			break;
		...
		if( access( file_ver2, F_OK ) != -1){ // se o ficheiro de saida já foi criado, não temos que o voltar a fazer
			...
			clock_gettime(CLOCK_MONOTONIC, &start_time_img); 
			vector[copy_t]++;	
			...
			// obter o tempo em que cada imagem já está guardada
			clock_gettime(CLOCK_MONOTONIC, &end_time_thread);
			*thread_time = diff_timespec(&end_time_thread, &start_time_img);
			fprintf(fpsaida,"%s    \t %10jd.%09ld\n", files[i], thread_time->tv_sec, thread_time->tv_nsec);
		}
	}
	clock_gettime(CLOCK_MONOTONIC, &end_time_thread);
...
int main(int argc, char*argv[]){
	...
	if (argc != 3){
		printf("o número de argumentos tem de ser 3!\n");
		return 0;
	}	
	if (argv[2]<0){
		printf("o número de threads não pode ser negativo!\n");
		return 0;
	}
	...
	copy_thread = THRS; // para saber quantas imagens são tratadas por cada thread(identificador)
	...
	fclose(fp);
    vector = calloc(sizeof(int*), THRS);
	for(int c = 0; c <THRS; c++){
		vector[c]=0;
	}
	...
	// código para guardar a imagem texture , que pode estar na pasta ou na diretoria
	...
	close(pipe_fd[1]); // fechar a pipe que escreve 
	...
	close(pipe_fd[0]); // fechar a pipe que lê 

	gdImageDestroy(data[0]->in_texture_img); // destruir a imagem textura
